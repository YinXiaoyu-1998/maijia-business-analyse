...
:root{{--bg:#f5f7fa;--surface:#fff;--ink:#172033;--muted:#657386;--line:#d9e2ea;--teal:#006d77;--blue:#2f5b9f;--green:#3a7d44;--amber:#b85c00;--red:#b23a48;--violet:#7557a6;--shadow:0 14px 34px rgba(23,32,51,.07);--radius:8px}}*{{box-sizing:border-box}}body{{margin:0;color:var(--ink);background:var(--bg);font:14px/1.55 -apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Microsoft YaHei",sans-serif}}main{{max-width:1360px;margin:0 auto;padding:26px 24px 64px}}.topbar{{position:sticky;top:0;z-index:10;background:rgba(245,247,250,.95);border-bottom:1px solid var(--line);backdrop-filter:blur(12px)}}.topbar-inner{{max-width:1360px;margin:0 auto;padding:12px 24px;display:flex;justify-content:space-between;gap:18px;align-items:center}}.brand{{font-weight:820;display:flex;align-items:center;gap:10px}}.mark{{width:30px;height:30px;border-radius:8px;background:linear-gradient(135deg,var(--teal),var(--blue))}}nav{{display:flex;flex-wrap:wrap;gap:6px}}nav a{{color:var(--muted);text-decoration:none;padding:7px 10px;border-radius:999px;font-size:13px}}nav a:hover{{background:var(--surface);color:var(--ink)}}.hero{{display:grid;grid-template-columns:minmax(0,1.3fr) minmax(320px,.7fr);gap:22px;align-items:stretch;padding:12px 0 24px}}.hero-summary{{background:#172033;color:#fff;border-radius:var(--radius);padding:20px;display:grid;gap:12px;box-shadow:var(--shadow)}}.hero-summary b{{font-size:28px;display:block;line-height:1.1;margin-top:6px}}.hero-summary span{{color:rgba(255,255,255,.72)}}.hero-summary .grid-2{{gap:12px}}.kicker{{color:var(--teal);font-size:12px;font-weight:800;letter-spacing:.04em;text-transform:uppercase}}h1{{font-size:38px;line-height:1.12;margin:8px 0 12px}}h2{{font-size:24px;line-height:1.24;margin:0}}h3{{font-size:16px;margin:0}}p{{line-height:1.65}}.lede{{color:var(--muted);max-width:780px;font-size:16px;margin:0}}.meta{{display:flex;flex-wrap:wrap;gap:8px;margin-top:18px}}.chip{{border:1px solid var(--line);background:var(--surface);color:var(--muted);padding:7px 10px;border-radius:999px}}.note,small{{color:var(--muted)}}.section{{border-top:1px solid var(--line);padding:30px 0}}.section-head{{display:flex;justify-content:space-between;align-items:end;gap:18px;margin-bottom:16px}}.panel-head{{display:flex;justify-content:space-between;gap:12px;align-items:center;margin-bottom:8px}}.grid-4{{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}}.grid-2{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}}.metric,.panel,.action{{background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);box-shadow:0 10px 24px rgba(23,32,51,.045)}}.metric{{padding:15px;min-height:120px}}.panel,.action{{padding:17px;min-width:0}}.metric span{{display:block;color:var(--muted);font-size:12px;font-weight:760}}.metric b{{display:block;font-size:28px;line-height:1;margin:8px 0}}.full{{margin-top:16px}}svg{{width:100%;height:auto}}svg text{{font-size:10px;fill:#657386}}.bar-row{{display:grid;grid-template-columns:86px 1fr 72px 92px;align-items:center;gap:9px;margin:15px 0;font-size:13px}}.bar-row i{{height:12px;background:#edf1f3;border-radius:999px;overflow:hidden}}.bar-row em{{display:block;height:100%;border-radius:999px}}.bar-row small{{text-align:right}}.table-wrap{{overflow:auto;border:1px solid var(--line);border-radius:var(--radius);background:var(--surface)}}table{{width:100%;min-width:780px;border-collapse:collapse;background:var(--surface)}}th,td{{padding:10px 11px;border-bottom:1px solid var(--line);text-align:right;font-size:13px;vertical-align:top;white-space:nowrap}}th{{background:#f0f4f7;color:var(--muted);font-size:12px;position:sticky;top:0}}th:first-child,td:first-child,td:last-child{{text-align:left}}td small{{display:block;margin-top:4px}}.move{{font-weight:760}}.actions{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}}.action p{{margin-bottom:0;color:var(--muted)}}.callout{{border-left:4px solid var(--amber);background:#fff8ef;border-radius:0 8px 8px 0;padding:14px 16px;color:#61420f}}@media(max-width:980px){{.hero,.grid-2,.actions{{grid-template-columns:1fr}}.grid-4{{grid-template-columns:repeat(2,minmax(0,1fr))}}.section-head{{align-items:start;flex-direction:column}}}}@media(max-width:620px){{main{{padding:18px 14px 48px}}.topbar-inner{{align-items:start;flex-direction:column}}.grid-4{{grid-template-columns:1fr}}h1{{font-size:30px}}nav{{display:none}}.bar-row{{grid-template-columns:70px 1fr 60px}}.bar-row small{{grid-column:2/5;text-align:left}}}}
</style></head><body><header class='topbar'><div class='topbar-inner'><div class='brand'><span class='mark'></span><span>麦家小馆周经营会报</span></div><nav><a href='#summary'>结论</a><a href='#trend'>趋势</a><a href='#stores'>门店明细</a><a href='#channels'>堂食外卖</a><a href='#people'>人效</a><a href='#products'>产品贡献</a><a href='#actions'>行动</a></nav></div></header><main>
<section class='hero'><div><div class='kicker'>Weekly Operating Review</div><h1>麦家小馆周经营会报</h1><p class='lede'>聚焦8月3日—9日三店经营、同比/环比、堂食/外卖、产品贡献和人效，识别可复制门店与优先复盘问题。</p><div class='meta'><span class='chip'>本周：2026/08/03—08/09</span><span class='chip'>环比：2026/07/27—08/02</span><span class='chip'>同比：2025/08/04—08/10</span><span class='chip'>门店：3家</span></div></div><aside class='hero-summary'><div><span>本周业务收入</span><b>{wan(ct['revenue'])}</b></div><div class='grid-2'><div><span>环比</span><b>{fp(rev_wow)}</b></div><div><span>同比</span><b>{fp(rev_yoy)}</b></div></div></aside></section>
