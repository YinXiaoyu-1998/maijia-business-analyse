#!/usr/bin/env python3
"""Run the monthly meeting report pipeline end to end."""
...
def main() -> None:
    ...
    profile_cmd = [
        sys.executable,
        str(script_dir / "profile_monthly_meeting_data.py"),
        "--input",
        *[str(path) for path in args.input],
        "--output-dir",
        str(args.output_dir),
        "--trend-months",
        str(args.trend_months),
    ]
    if args.dish_input:
    ...
    run([
        sys.executable,
        str(script_dir / "generate_monthly_meeting_report_html.py"),
        "--input-dir",
        str(args.output_dir),
        "--output",
        str(args.report),
