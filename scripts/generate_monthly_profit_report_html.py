    ...
    for store in stores:
        ...
        for year in years:
            ...
            data[store][str(year)] = {
                ...
                "margin": [{"month": row["月份"], "value": row["利润率"], "status": row["利润率状态"], "revenue": row["店内营业收入"]} for row in complete],
            }
    return {
        "meta": {
...
<head>
  ...
  <style>
    :root { --ink:#14213d; --muted:#64748b; --line:#dbe4ee; --teal:#087c85; --yellow:#f2c94c; --yellow-dark:#c89b18; --blue:#2f80ed; --blue-dark:#1c5fb8; --red:#e5484d; --red-dark:#b42318; --bg:#f6f8fb; --card:#fff; }
    * { box-sizing:border-box; } body { margin:0; background:var(--bg); color:var(--ink); font-family:-apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei",sans-serif; }
    ...
    .chart-wrap { position:relative; padding:4px 10px 12px; } svg { display:block; width:100%; height:auto; overflow:visible; } .axis { font-size:10px; fill:var(--muted); } .grid-line { stroke:var(--line); stroke-width:1; } .zero { stroke:#93a6b8; stroke-width:1.25; } .trend { fill:none; stroke:var(--yellow-dark); stroke-width:3; stroke-linecap:round; stroke-linejoin:round; } .dot { fill:var(--yellow); stroke:var(--yellow-dark); stroke-width:2.5; cursor:default; } .min-dot { fill:var(--red); stroke:var(--red-dark); } .max-dot { fill:var(--blue); stroke:var(--blue-dark); } .empty { fill:#94a3b8; font-size:13px; font-weight:650; }
...
<body><main>
  ...
  <section><div class="section-head"><div><h2>月利润率趋势</h2></div><p>利润率 = 净利润 ÷ 当月店内营业收入汇总；纵轴为百分比。</p></div><div class="grid" id="marginGrid"></div></section>
...
function renderLine(host,store,type,year){ const rows=REPORT.data[store][String(year)][type]; const w=450,h=260,l=52,r=16,t=20,b=43,pw=w-l-r,ph=h-t-b; const vals=rows.filter(x=>x.value!==null).map(x=>x.value); const scaleTicks=vals.length?ticks(Math.min(...vals),Math.max(...vals)):[]; const y=v=>t+(scaleTicks.at(-1)-v)/(scaleTicks.at(-1)-scaleTicks[0])*ph; const x=m=>l+(m-1)*pw/11; const formatter=type==='profit'?money:percent; const label=type==='profit'?'净利润':'利润率'; let svg=`<svg viewBox="0 0 ${w} ${h}" role="img" aria-label="${store}${year}年${label}折线图">`;
 if(!vals.length){svg+=`<text x="${w/2}" y="${h/2}" text-anchor="middle" class="empty">该年度暂无可展示数据</text>`;} else {scaleTicks.forEach((v,i)=>{const yy=t+ph-i*ph/4;svg+=`<line x1="${l}" x2="${w-r}" y1="${yy}" y2="${yy}" class="${Math.abs(v)<1e-9?'zero':'grid-line'}"/><text x="${l-7}" y="${yy+4}" text-anchor="end" class="axis">${type==='profit'?(Math.abs(v)>=10000?(v/10000).toFixed(1)+'万':Math.round(v)):percent(v)}</text>`;}); const plotted=rows.filter(p=>p.value!==null); const minValue=Math.min(...vals),maxValue=Math.max(...vals),hasDistinctExtremes=plotted.length>1&&minValue!==maxValue; const segments=[];let current=[];rows.forEach(p=>{if(p.value===null){if(current.length){segments.push(current);current=[];}}else current.push(p);});if(current.length)segments.push(current);segments.forEach(seg=>svg+=`<path class="trend" d="${seg.map((p,i)=>`${i?'L':'M'}${x(p.month).toFixed(1)},${y(p.value).toFixed(1)}`).join(' ')}"/>`); plotted.forEach(p=>{const extremeClass=!hasDistinctExtremes?'':p.value===minValue?'min-dot':p.value===maxValue?'max-dot':'';svg+=`<circle class="dot ${extremeClass}" data-month="${p.month}" cx="${x(p.month)}" cy="${y(p.value)}" r="6.5"/>`;}); }
 M.forEach(m=>{const xx=x(m);svg+=`<text x="${xx}" y="${h-16}" text-anchor="middle" class="axis">${m}月</text>`;}); svg+='</svg>'; host.innerHTML=svg; host.querySelectorAll('.dot[data-month]').forEach(dot=>{const p=rows.find(x=>x.month===Number(dot.dataset.month)); const detail=`<b>${year} 年 ${p.month} 月</b>${label}：${formatter(p.value)}${type==='margin'&&p.revenue!==null?`<br>店内营业收入：${money(p.revenue)}`:''}`; dot.addEventListener('mouseenter',e=>{tip.innerHTML=detail;tip.style.display='block';tip.style.left=(e.clientX+12)+'px';tip.style.top=(e.clientY+12)+'px';});dot.addEventListener('mousemove',e=>{tip.style.left=(e.clientX+12)+'px';tip.style.top=(e.clientY+12)+'px';});dot.addEventListener('mouseleave',()=>tip.style.display='none');}); }
document.getElementById('method').textContent=REPORT.meta.rate_rule; document.getElementById('footer').textContent=`生成日期：${REPORT.meta.generated} ｜ 利润来源：${REPORT.meta.profit_source} ｜ 店内营业收入来源：${REPORT.meta.business_file_count} 个营业分组表导出文件`;
