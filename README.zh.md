...
- 周报和月报提供两张可检索的 `产品万元销量` 表，分别使用 `订单营业收入` 与 `营业额(元)` 作为分母，均支持全体门店与单店切换。
