...
周报和月报使用营业导出的 `时段` 字段做时段归因。展示 `档口占比` 时，还需要按 `自助取数 -> 自助菜品取数` 流程导出菜品主题数据，并同时导出菜品库，分别按 `documents/raw_exports/maijia_dishes_20260614_20260620.xlsx` 和 `documents/raw_exports/maijia_dish_catalog_20260620.xlsx` 命名。分母为 `营业分组表.店内营业收入`；分子为 `订单分类=店内销售` 的 `菜品收入`，先按 `菜品名称`、再用 `关联菜品名称` 兜底映射到 `总部菜品.基础分类`。展示 Top 10 已匹配档口，并把 `未匹配` 与 `其他` 分开列示。
