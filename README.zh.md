...
周报和月报使用营业导出的 `时段` 字段做时段归因。当报表需要展示 `销售额菜品比例` 饼图时，还需要按同一参考文件里的 `自助取数 -> 自助菜品取数` 流程导出菜品主题数据。展开筛选并全选字段组，导出后在 `下载清单记录` 中下载匹配的 `菜品主题数据(日期【...】)` 行，并按标准命名保存，例如 `documents/raw_exports/maijia_dishes_20260614_20260620.xlsx`。该饼图以 `营业分组表.店内营业收入` 为分母，以 `订单分类=店内销售` 的 `菜品收入` 为分子，不需要菜品库。
