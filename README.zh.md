...
- 周报和月报会基于 `营业分组表` 的 `时段` 字段生成时段归因。
- 周报和月报提供基于 `自助菜品取数` 与菜品库的 `档口占比` 饼图和明细表。
- 周报和月报提供两张可检索的 `产品万元销量` 表；菜品销量按 `堂食` / `外卖` 拆行但共享所选门店范围的全渠道分母，两表分别使用 `订单营业收入` 与 `营业额(元)`，均支持全体门店与单店切换。
...
周报和月报使用营业导出的 `时段` 字段做时段归因。展示 `档口占比` 或 `产品万元销量` 时，还需要按 `自助取数 -> 自助菜品取数` 流程导出菜品主题数据，并同时导出菜品库，分别按 `documents/raw_exports/maijia_dishes_20260614_20260620.xlsx` 和 `documents/raw_exports/maijia_dish_catalog_20260620.xlsx` 命名。`档口占比` 维持店内收入口径。两张产品万元销量表只纳入当前营业分母范围内的门店，并把原始 `订单分类` 精确等于 `店内销售` 的销量归为 `堂食`，其余所有值归为 `外卖`；两类销量行仍共享所选门店范围、当前区间的全渠道总分母。`产品万元销量（订单营业收入）` 使用 `订单营业收入`，`产品万元销量（营业额）` 使用 `营业额(元)`；全体门店先跨店求和再相除。产品名优先使用 `关联菜品名称`，为空时回退 `菜品名称`；表格增加 `销售分类` 列，搜索可同时返回同名堂食和外卖行。
